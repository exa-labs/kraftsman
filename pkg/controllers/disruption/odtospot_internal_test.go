...
func TestRetrySpotOnlyReplacementsExcludesSpikedZones(t *testing.T) {
	...
	if ok, reason, _ := c.filterReplacementsAndPublish([]*pscheduling.NodeClaim{nc}, nil, priceBudget{candidatePrice: 13.0}, false); ok || reason != CandidateSkipNoCheaperSingleReplacement {
		t.Fatalf("filterReplacementsAndPublish() = (%t, %q), want rejection with %q", ok, reason, CandidateSkipNoCheaperSingleReplacement)
	}
	if !c.retrySpotOnlyReplacements(context.Background(), "", consolidationSimulationOptions{}, nil, []*pscheduling.NodeClaim{nc}, snapshot, priceBudget{candidatePrice: 13.0}, MinInstanceTypesForSpotToSpotConsolidation, nil) {
...
func TestRetrySpotOnlyReplacementsUsesPerClaimBudgetForSplits(t *testing.T) {
	...
	if !c.retrySpotOnlyReplacements(context.Background(), "", consolidationSimulationOptions{}, nil, []*pscheduling.NodeClaim{nc1, nc2}, snapshots, priceBudget{candidatePrice: 13.0}, MinInstanceTypesForSpotToSpotConsolidation, nil) {
...
func TestODToSpotRetryApplies(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			name: "shadow mode applies without enabling the feature",
			ctx: options.ToContext(context.Background(), &options.Options{
				ODToSpotConsolidation:       false,
				ODToSpotConsolidationShadow: true,
			}),
			candidates: []*Candidate{{capacityType: v1.CapacityTypeOnDemand}},
			claims:     []*pscheduling.NodeClaim{spotCapable},
			want:       true,
		...
		t.Run(tc.name, func(t *testing.T) {
			if got, shadowOnly := c.odToSpotRetryApplies(tc.ctx, tc.candidates, tc.claims); got != tc.want || shadowOnly != tc.wantShadow {
				t.Errorf("odToSpotRetryApplies() = (%t, %t), want (%t, %t)", got, shadowOnly, tc.want, tc.wantShadow)
			}
		})
	}
}
