...
func (o *Options) AddFlags(fs *FlagSet) {
	...
	fs.DurationVar(&o.DisruptionUnprovisionablePodTTL, "disruption-unprovisionable-pod-ttl", env.WithDefaultDuration("DISRUPTION_UNPROVISIONABLE_POD_TTL", 2*time.Minute), "How long a pending pod that the provisioner's most recent scheduling simulation could not place anywhere stays excluded from disruption scheduling simulations. Such a pod cannot land on any node the simulation might open, so simulating it only spends the candidate's budget; the verdict is refreshed by every provisioning pass and cleared as soon as one places the pod. The TTL bounds staleness when provisioning stalls: a verdict older than it is ignored and the pod is simulated again. 0 disables the exclusion.")
