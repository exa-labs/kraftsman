...
func (o *Options) validateConsolidation() error {
	...
	if o.ConsolidationSplitShadowMaxReplacements < 2 {
		return fmt.Errorf("validating cli flags / env vars, CONSOLIDATION_SPLIT_SHADOW_MAX_REPLACEMENTS must be >= 2, got %d", o.ConsolidationSplitShadowMaxReplacements)
	}
...
func (o *Options) validateNegativeCache() error {
	if o.DisruptionUnprovisionablePodTTL < 0 {
		return fmt.Errorf("validating cli flags / env vars, DISRUPTION_UNPROVISIONABLE_POD_TTL must be >= 0, got %s", o.DisruptionUnprovisionablePodTTL)
	}
